...
/**
 * Creates a collection's indexes at most once per process. A rejection clears
 * the cache so the next request retries instead of replaying the failure.
 */
async function withIndexes(cacheKey, collection, specs) {
  globalThis[cacheKey] ??= collection.createIndexes(specs).catch((error) => {
    globalThis[cacheKey] = undefined;
    throw error;
  });
  await globalThis[cacheKey];
  return collection;
}

export async function getBookingsCollection() {
  const db = await getDb();
  return withIndexes("__busMongoBookingIndexes", db.collection(COLLECTIONS.bookings), BOOKING_INDEXES);
}

/** Admin accounts: one document per admin, password stored scrypt-hashed. */
export async function getAdminsCollection() {
  const db = await getDb();
  return withIndexes("__busMongoAdminIndexes", db.collection(COLLECTIONS.admins), [
    { key: { email: 1 }, name: "admin_email_unique", unique: true },
  ]);
}
